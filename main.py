...
class Villages:
    def __init__(self):
        self.spritesheet = pg.image.load('img/village-sprites.png')
        self.sprites = [(66, 0, 33, 34), (128, 0, 33, 44), (99, 0, 33, 44), (162, 0, 41, 39), (200, 0, 35, 50)]
        self.locations = []
        for y in range(0, len(self.sprites)):
            for x in range (0, 5):
                self.locations = self.locations + [pg.Rect(80+x*100, 80+y*100, self.sprites[x][2], self.sprites[x][3])]
        print("Number of Total Villages: " + str(len(self.locations)))
...
class Game(States, Villages):
    def __init__(self):
        States.__init__(self)
        self.villages = Villages()
        self.next = 'menu'
        self.selected_village = 0
    def cleanup(self):
        print('cleaning up Game state stuff')
    def startup(self):
        print('starting Game state stuff')
    def village_select(self, mouse_x, mouse_y):
        for i in range(0, len(self.villages.locations)):
            print(str(i))
            print("Collision:" + str(self.villages.locations[i].collidepoint(mouse_x, mouse_y)))
            if self.villages.locations[i].collidepoint(mouse_x, mouse_y):
                self.selected_village = i
    def get_event(self, event):
        if event.type == pg.KEYDOWN:
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            self.village_select(mouse_x, mouse_y)
    def update(self, screen, dt):
        self.draw(screen)
    def draw(self, screen):
        screen.fill((62,145,51))
        for v in range(0, len(self.villages.sprites)):
            for x in range(0, 4):
                screen.blit(self.villages.spritesheet, (80+v*100, 80+x*100), self.villages.sprites[v])
        pg.draw.rect(screen, (195, 0, 0), self.villages.locations[self.selected_village], 3)
...
class Battle(States):
    def __init__(self):
        ...
        self.next = 'game'
    def cleanup(self):
        print('cleaning up Battle state stuff')
    def startup(self):
        print('starting Battle state stuff')
    def get_event(self, event):
